...
- Idle bar 40 DIP; activity bar 56 DIP. A stable 56-DIP host prevents capture reallocations during height animation. Work-area reservation grows before activity presentation and shrinks after collapse.
...
- Native taskbar retains pinned/running apps, jump lists, and previews. Windhawk's DockLike theme makes the Explorer taskbar hug the application buttons in a fully rounded dock. The Taskbar height and icon size mod raises the reserved strip to 56 DIP: 4-DIP outer gaps above and below surround a 48-DIP dock, whose 4-DIP internal padding leaves 40-DIP square native button cells intact. Running indicators are translated 2 DIP toward the dock edge to separate them from the icons. Individual tray controls are transparent and non-interactive while their measured layout stays alive, which preserves DockLike's auto-width calculation on Windows build 26200. The top bar invokes Explorer's native tray overflow, Quick Settings, and notification center. Native notification banners remain enabled.
