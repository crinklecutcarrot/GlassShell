...
- Native taskbar retains pinned/running apps, jump lists, and previews. Windhawk's DockLike theme makes the Explorer taskbar hug the application buttons in a fully rounded dock. The native 48-DIP taskbar strip is divided into 4-DIP outer gaps above and below, 4-DIP inner padding on every edge, and 32 DIP for icon content. Individual tray controls are transparent and non-interactive while their measured layout stays alive, which preserves DockLike's auto-width calculation on Windows build 26200. The top bar opens Explorer's native overflow when tray access is needed. Connected notification history remains future work. Native notification banners remain enabled.
