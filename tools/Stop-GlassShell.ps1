...
"@
$script:glassPid = 0
[GlassStop]::EnumWindows({param($h,$l) $title=New-Object Text.StringBuilder 256;[void][GlassStop]::GetWindowText($h,$title,256);if($title.ToString() -eq 'GlassShell · Status'){[void][GlassStop]::GetWindowThreadProcessId($h,[ref]$script:glassPid);[void][GlassStop]::PostMessage($h,0x312,[IntPtr]1,[IntPtr]::Zero)};return $true},[IntPtr]::Zero)|Out-Null
if($script:glassPid){$glassProcess=Get-Process -Id $script:glassPid -ErrorAction SilentlyContinue;if($glassProcess -and -not $glassProcess.WaitForExit(5000)){throw 'App did not exit gracefully'}}
