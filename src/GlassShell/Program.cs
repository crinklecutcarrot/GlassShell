...
{
    public ShellModel Model { get; } = new(); public MediaService Media { get; } = new(); public TrayService Tray { get; } = new(); public ConnectivityService Connectivity { get; } = new(); public AudioService Audio { get; } = new();
    ...
    readonly Stopwatch watch = Stopwatch.StartNew(); double lastMedia, lastConnectivity; bool disposed, fullscreen, expired; IntPtr foreground;
    OutsideClickMonitor? outside; System.Windows.Forms.NotifyIcon? tray; TimeSpan lastFrame;
    public ShellController() { Bar = new(this); Panel = new(this); Alert = new(this); Model.Changed += Update; Media.Changed += Update; Tray.Changed += TrayChanged; Connectivity.Changed += ConnectivityChanged; }
    public void Start()
    {
