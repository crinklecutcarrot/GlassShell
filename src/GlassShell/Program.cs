...
internal sealed class ShellController : IDisposable
{
    ...
    OutsideClickMonitor? outside; ScreenshotShortcutMonitor? screenshotShortcut; System.Windows.Forms.NotifyIcon? tray; TimeSpan lastFrame;
    ...
    public void Start()
    {
        Bar.Show(); timer.Tick += Tick; timer.Start(); CompositionTarget.Rendering += Render; outside = new(CaptureOutsidePress); try { screenshotShortcut = new(PrepareScreenshot); } catch (Exception e) { Storage.Log("Screenshot shortcut monitor: " + e.Message); }
    ...
    void UpdateContext()
    {
        ...
        if (snipping) screenshotUntil = Math.Max(screenshotUntil, watch.Elapsed.TotalSeconds + 2.5); bool screenshotActive = snipping || watch.Elapsed.TotalSeconds < screenshotUntil; SetScreenshotMode(screenshotActive);
    ...
    void PrepareScreenshot() { screenshotUntil = Math.Max(screenshotUntil, watch.Elapsed.TotalSeconds + 2); SetScreenshotMode(true); }
    void SetScreenshotMode(bool active)
    {
        if (screenshotMode == active) return; screenshotMode = active; foreach (var w in new ShellWindow[] { Bar, Panel, Alert }) if (w.Handle != IntPtr.Zero) Native.SetWindowDisplayAffinity(w.Handle, active || !LiveGlass ? 0u : 0x11u); Storage.Log(active ? "Screenshot mode: froze glass before the snipping shortcut" : "Screenshot mode: restored live capture exclusion"); if (!active && LiveGlass) foreach (var w in new ShellWindow[] { Bar, Panel, Alert }) w.RefreshGlass();
    }
    ...
    public void Dispose() { if (disposed) return; disposed = true; timer.Stop(); CompositionTarget.Rendering -= Render; outside?.Dispose(); screenshotShortcut?.Dispose(); Panel.SaveNote(); Tray.Dispose(); Connectivity.Dispose(); Bar.Unregister(); tray?.Dispose(); Media.Dispose(); Panel.Close(); Alert.Close(); Bar.Close(); Storage.Log("Stopped status-bar layout"); }
}
