...
internal sealed class UiRegressionTest
{
    ...
    public async Task Run()
    {
        var checks = new Dictionary<string, bool>(); string? error = null; Window? fixture = null; Native.GetCursorPos(out var originalCursor); try
        {
            ...
            int clicks = 0; var target = Ui.Button("Test click target", () => clicks++, 280, 120); fixture = new Window { Left = 80, Top = 500, Width = 300, Height = 160, Content = target, Topmost = true, ShowInTaskbar = false, ShowActivated = false }; fixture.Show(); await Task.Delay(200); long before = s.ObservedPresses; await Press(target.PointToScreen(new Point(100, 60))); checks["native outside click propagates"] = clicks == 1 && s.ObservedPresses > before && !s.Panel.IsOpen; if (s.Panel.IsOpen) s.Panel.Dismiss();
            await Press(new Point(s.Bar.AnchorCenter("music") * s.Bar.Scale, (s.Bar.Top + 24) * s.Bar.Scale)); checks["music surface click opens dropdown"] = s.Panel.IsOpen && s.Panel.CurrentPage == "music"; if (!s.Panel.IsOpen) { s.OpenPanel("music"); await Task.Delay(220); }
            ...
            var musicBody = (StackPanel)s.Panel.Glass.Content.Children[^1]; var detailBackdrop = (Image)s.Panel.Glass.Content.Children[0]; var detailOverlay = (Border)s.Panel.Glass.Content.Children[1]; var detailLabels = FindVisual<StackPanel>(musicBody, x => x.Children.OfType<TextBlock>().Any(t => t.Text == "A test track") && x.Children.OfType<TextBlock>().Any(t => t.Text == "Test artist"))!; var detailLike = FindVisual<Button>(musicBody, b => b.ToolTip?.ToString()?.Contains("YouTube Music", StringComparison.Ordinal) == true)!; var detailsRow = (Grid)VisualTreeHelper.GetParent(detailLike); double playerLeftInset = musicBody.TranslatePoint(new Point(), s.Panel).X; double playerBottomInset = s.Panel.Height - musicBody.TranslatePoint(new Point(0, musicBody.ActualHeight), s.Panel).Y; checks["album art fills blurred centered player background"] = detailBackdrop.Stretch == Stretch.UniformToFill && detailBackdrop.HorizontalAlignment == HorizontalAlignment.Center && detailBackdrop.RenderTransformOrigin == new Point(.5, .5) && detailBackdrop.RenderTransform is ScaleTransform scale && scale.ScaleX > 1 && detailBackdrop.Effect is System.Windows.Media.Effects.BlurEffect blur && blur.Radius >= 30; checks["player artwork has uniform dark overlay"] = detailOverlay.Background is SolidColorBrush overlay && overlay.Color.A >= 160; checks["player has even inner bottom padding"] = Math.Abs(playerBottomInset - playerLeftInset) <= 2; checks["player keeps foreground album art"] = FindVisual<RoundedImage>(musicBody)?.Width == 48; checks["heart aligns to content edge"] = Math.Abs(detailLike.TranslatePoint(new Point(detailLike.ActualWidth, 0), detailsRow).X - detailsRow.ActualWidth) <= 1.1; checks["heart has no idle fill"] = detailLike.Background == Brushes.Transparent; checks["now playing window is compact"] = s.Panel.Width == 350;
            s.Media.SetTestTrack("Next test track", "Next artist", 1); await Task.Delay(70);
            checks["next track exits left"] = ((TranslateTransform)detailLabels.RenderTransform).X < 0;
            await Task.Delay(560); checks["next track enters and settles"] = FindText(musicBody, "Next test track");
            s.Media.SetTestTrack("Previous test track", "Previous artist", -1); await Task.Delay(52);
            checks["previous track exits right"] = ((TranslateTransform)detailLabels.RenderTransform).X > 0;
            ...
            var queueButton = FindVisual<Button>(s.Panel, b => FindText(b, "Queue")); if (queueButton != null) await Press(queueButton.PointToScreen(new Point(queueButton.ActualWidth / 2, queueButton.ActualHeight / 2))); checks["queue button opens subpage"] = s.Panel.CurrentPage == "queue"; checks["queue shows past current and upcoming tracks"] = FindText(s.Panel, "Earlier test track") && FindText(s.Panel, "A test track") && FindText(s.Panel, "Next test track"); checks["queue keeps compact player controls"] = FindVisual<Button>(s.Panel, b => Equals(b.ToolTip, "Play / pause")) != null; var queueNowTitle = FindVisual<TextBlock>(s.Panel, t => t.Text == "Previous test track"); var upNextLabel = FindVisual<TextBlock>(s.Panel, t => t.Text == "Up next"); checks["queue current track aligns with content"] = queueNowTitle != null && upNextLabel != null && Math.Abs(queueNowTitle.PointToScreen(new Point()).X - upNextLabel.PointToScreen(new Point()).X) < 1; checks["queue rows use artwork and drag handles"] = FindVisual<RoundedImage>(s.Panel)?.Width == 42 && FindVisual<Button>(s.Panel, b => Equals(b.ToolTip, "Drag to reorder")) != null; var pastQueueRow = FindVisual<Border>(s.Panel, b => b.Tag is QueueTrack q && q.title == "Earlier test track"); var currentQueueRow = FindVisual<Border>(s.Panel, b => b.Tag is QueueTrack q && q.selected); var queueViewport = FindVisual<ScrollViewer>(s.Panel); checks["past queue item is dimmed"] = pastQueueRow?.Opacity < .5; checks["current queue item is active"] = currentQueueRow?.Background == Ui.WindowsAccentSurface; checks["queue defaults current song to top"] = currentQueueRow != null && queueViewport != null && Math.Abs(currentQueueRow.TranslatePoint(new Point(), queueViewport).Y) < 2; if (queueViewport != null) { queueViewport.ScrollToBottom(); await Task.Delay(180); checks["queue incrementally renders deeper tracks"] = FindText(s.Panel, "Queued test track 40"); } await Capture(s.Panel, "music-queue.png"); s.Panel.Navigate("music"); await Task.Delay(200);
            ...
            s.OpenPanel("controls"); await Task.Delay(220); var controlTiles = FindVisual<StackPanel>(s.Panel, x => x.Orientation == Orientation.Horizontal && x.Children.Count == 2 && x.Children[0] is Button b && b.ActualWidth > 160); var wifiTile = controlTiles?.Children[0] as Button; checks["active connectivity tile uses Windows accent"] = wifiTile?.Background == Ui.WindowsAccentSurface; await Capture(s.Panel, "control-center.png");
    ...
    static T? FindVisual<T>(DependencyObject parent, Func<T, bool>? predicate = null) where T : DependencyObject
    {
        if (parent is T match && (predicate == null || predicate(match))) return match;
        for (int i = 0; i < VisualTreeHelper.GetChildrenCount(parent); i++) { var found = FindVisual(VisualTreeHelper.GetChild(parent, i), predicate); if (found != null) return found; }
        return null;
    }
}
