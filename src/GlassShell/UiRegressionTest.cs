...
internal sealed class UiRegressionTest
{
    ...
    public async Task Run()
    {
        var checks = new Dictionary<string, bool>(); string? error = null; Window? fixture = null; Native.GetCursorPos(out var originalCursor); try
        {
            ...
            var musicBody = (StackPanel)s.Panel.Glass.Content.Children[^1]; var detailBackdrop = (Image)s.Panel.Glass.Content.Children[0]; var detailLabels = FindVisual<StackPanel>(musicBody, x => x.Children.OfType<TextBlock>().Any(t => t.Text == "A test track") && x.Children.OfType<TextBlock>().Any(t => t.Text == "Test artist"))!; var detailLike = FindVisual<Button>(musicBody, b => b.ToolTip?.ToString()?.Contains("YouTube Music", StringComparison.Ordinal) == true)!; var detailsRow = (Grid)VisualTreeHelper.GetParent(detailLike); checks["album art fills blurred player background"] = detailBackdrop.Stretch == Stretch.UniformToFill && detailBackdrop.Width > s.Panel.Width && detailBackdrop.Effect is System.Windows.Media.Effects.BlurEffect blur && blur.Radius >= 30; checks["player keeps foreground album art"] = FindVisual<RoundedImage>(musicBody)?.Width == 48; checks["heart aligns to content edge"] = Math.Abs(detailLike.TranslatePoint(new Point(detailLike.ActualWidth, 0), detailsRow).X - detailsRow.ActualWidth) <= 1.1; checks["heart has no idle fill"] = detailLike.Background == Brushes.Transparent; checks["now playing window is compact"] = s.Panel.Width == 350;
