...
internal sealed class UiRegressionTest
{
    ...
    public async Task Run()
    {
        var checks = new Dictionary<string, bool>(); string? error = null; Window? fixture = null; Native.GetCursorPos(out var originalCursor); try
        {
            ...
            var center = (StackPanel)s.Bar.Glass.Content.Children[2]; var music = (Button)center.Children[0]; checks["selected music highlight"] = ((SolidColorBrush)((Border)music.Template.FindName("ButtonSurface", music)).Background).Color.A == 54; var musicRow = (StackPanel)music.Content; checks["compact album art is rounded"] = ((Image)((RoundedImage)musicRow.Children[0]).Child).Clip is RectangleGeometry compactClip && compactClip.RadiusX == 6; var transport = (Button)musicRow.Children[2]; await Press(transport.PointToScreen(new Point(14, 14))); checks["bar transport does not toggle music menu"] = s.Panel.IsOpen && s.Panel.CurrentPage == "music";
            ...
            var activeModule = (Button)center.Children[2]; checks["timer has even card padding"] = activeModule.Padding == new Thickness(8); double width = activeModule.ActualWidth; s.Model.Advance(TimeSpan.FromMinutes(20)); await Task.Delay(80); checks["timer module stable width"] = activeModule.ActualWidth == width;
            s.Panel.Dismiss();
            ...
            foreach (var page in new[] { "widgets", "timer", "notes", "controls", "notifications", "tray" }) { s.OpenPanel(page); await Task.Delay(180); if (page == "notes") { foreach (var child in ((System.Windows.Controls.StackPanel)s.Panel.Glass.Content.Children[0]).Children) if (child is TextBox note) { note.Text = "Isolated autosave check"; checks["note autosaves"] = Storage.Read("quick-note.txt") == note.Text; } } if (page == "tray") { var trayBody = (StackPanel)s.Panel.Glass.Content.Children[0]; var iconGrid = trayBody.Children.OfType<WrapPanel>().First(); var trayButton = (Button)iconGrid.Children[0]; await Press(trayButton.PointToScreen(new Point(20, 20))); checks["mirrored tray click forwards"] = s.Tray.LastTestAction == "left"; await Capture(s.Panel, "tray-manager.png"); } checks[page + " opens below bar"] = s.Panel.IsOpen && s.Panel.Top + 1 >= s.Bar.Top + s.Bar.VisualHeight; s.Panel.Dismiss(); await Task.Delay(150); checks[page + " dismisses"] = !s.Panel.IsOpen; }
